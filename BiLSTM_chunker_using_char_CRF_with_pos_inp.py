...
def create_word_char_pos_dicts(totaldata):
    ...
    for sent,tag in tot_data:
        ...
        for tg in tag:
            if tg[0] not in tag_to_ix.keys():
                tag_to_ix[tg[0]] = len(tag_to_ix)

        for tg in tag:
            if tg[1] not in inp_tag_to_ix.keys():
                inp_tag_to_ix[tg[1]] = len(inp_tag_to_ix)
...
with open("total_poschunk_data.pickle","rb") as fhnd:
...
word_to_ix=load(open("word_to_idx.pickle","rb"))
char_to_ix=load(open("char_to_idx.pickle","rb"))
tag_to_ix=load(open("chunk_map.pickle","rb"))
inp_tag_to_ix=load(open("inp_pos_map.pickle","rb"))
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, dist_char_size,dist_posinp_size,tag_to_ix, embedding_dim, hidden_dim,char_embedding_dim,posinp_embedding_dim):
        super(BiLSTM_CRF, self).__init__()
        self.embedding_dim = embedding_dim
        self.char_embedding_dim=char_embedding_dim
        self.posinp_embedding_dim=posinp_embedding_dim
        self.hidden_dim = hidden_dim
        self.vocab_size = vocab_size
        self.tag_to_ix = tag_to_ix
        self.tagset_size = len(tag_to_ix)

        self.posinp_embeddings=nn.Embedding(dist_posinp_size,self.posinp_embedding_dim)
        ...
        self.lstm = nn.LSTM(embedding_dim+(self.char_embedding_dim*2+self.posinp_embedding_dim), hidden_dim,
    ...
    def _get_lstm_features(self, sentence,posinp):
        self.hidden = self.init_hidden()
...
#         embeds = self.word_embeds(torch.tensor(only_words).cuda()).view(len(only_words), 1, -1).cuda()
        ...
        posinp_embeds = self.posinp_embeddings(torch.tensor(posinp).cuda()).view(len(posinp), 1, -1).cuda()
        final_input = torch.cat((embeds,char_lstm_outputs,posinp_embeds),-1)
    ...
    def neg_log_likelihood(self, sentence,posinp, tags):
        feats = self._get_lstm_features(sentence,posinp)
        forward_score = self._forward_alg(feats)
        gold_score = self._score_sentence(feats, tags)
        return forward_score - gold_score

    def forward(self, sentence,posinp):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        lstm_feats = self._get_lstm_features(sentence,posinp)
...
model = BiLSTM_CRF(len(word_to_ix),len(char_to_ix),len(inp_tag_to_ix), tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM,char_embedding_dim,pos_embedding_dim)
model=model.cuda()
...
for epoch in tqdm_notebook(range((backup_point+1),num_epochs),desc='epochs'):  # again, normally you would NOT do 300 epochs, it is toy data
...
#             print("Error in saving the model , ",e)
    ...
    for sentence, tags in tqdm_notebook(training_data,desc='Trng examples',leave=False):
        ...
        precheck_posinp = [inp_tag_to_ix[t[1]] for t in tags]

        # Step 3. Run our forward pass.
        loss = model.neg_log_likelihood(sentence_in,precheck_posinp, targets)
...
dump(word_to_ix,open("word_to_idx.pickle","wb"))
dump(char_to_ix,open("char_to_idx.pickle","wb"))
dump(tag_to_ix,open("chunk_map.pickle","wb"))
dump(inp_tag_to_ix,open("inp_pos_map.pickle","wb"))
...
total=0
with torch.no_grad():
    for v in tqdm_notebook(range(len(test_data))):
        if '' in test_data[v][0]:continue
        precheck_sent = generate_sequence(test_data[v][0], word_to_ix,char_to_ix)
        precheck_tags = torch.tensor([tag_to_ix[t[0]] for t in test_data[v][1]], dtype=torch.long)
        precheck_posinp = [inp_tag_to_ix[t[1]] for t in test_data[v][1]]
        predicted=model(precheck_sent,precheck_posinp)
